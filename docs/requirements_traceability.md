...
| Retry/ACK/dedup | aplicações drone/equipe; `alertId` evita novo atendimento e `messageId` identifica duplicatas | `Validation_Direct`, `Validation_Obstacle_BaOff`, `Validation_BaOn` |
...
| BA e aptidão | `BatAlgorithm`/drone app; `docs/bat_algorithm_and_fitness.md` | seed fixa, limites e métricas de resultado |
